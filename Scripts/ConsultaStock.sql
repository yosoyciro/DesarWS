...
BEGIN
	...
	CASE V.TIPOTRANSMISION WHEN 'M' THEN 'MANUAL' WHEN 'A' THEN 'AUTOMATICA' ELSE 'NO ESPECIFICADO' END TIPOCAJA, CA.DESCRIPCION CATEGORIA, dbo.PrecioArticulo(S.ARTICULOSID, V.CATEGORIASID, V.TIPOTRANSMISION) PRECIOVENTA, V.PATENTE
	...
	INNER JOIN Colores C ON V.COLORESID = C.COLORESID
	INNER JOIN TiposCombustible TC ON TC.TIPOSCOMBUSTIBLEID = V.TIPOSCOMBUSTIBLEID
	INNER JOIN Categorias CA ON V.CATEGORIASID = CA.CATEGORIASID
	WHERE V.MARCASID = @pMarca AND V.MODELOSID = @pModelo AND (0 = @pArticulo OR S.ARTICULOSID = @pArticulo)
